...
{
	solAssert(_targetOffset + _size <= _target.size(), "");
	for (size_t i = 0; i < _size; ++i)
		_target[_targetOffset + i] = _sourceOffset + i < _source.size() ? 0 : _source[_sourceOffset + i];
}
...
)
{
	...
	solAssert(size_t(info.args) == _arguments.size(), "");
	...
	case Instruction::DIV:
		return arg[1] == 0 ? 0 : divWorkaround(arg[0],  arg[1]);
	case Instruction::SDIV:
		return arg[1] == 0 ? 0 : s2u(divWorkaround(u2s(arg[0]), u2s(arg[1])));
	case Instruction::MOD:
		return arg[1] == 0 ? 0 : modWorkaround(arg[0],  arg[1]);
	case Instruction::SMOD:
		return arg[1] == 0 ? 0 : s2u(modWorkaround(u2s(arg[0]), u2s(arg[1])));
