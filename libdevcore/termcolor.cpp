...
bool stdoutInitialized = initialize(STDERR_FILENO);
bool stderrInitialized = initialize(STDERR_FILENO);

}  // namespace internal

bool initialize(int terminalFd)
{
	if (terminalFd != STDOUT_FILENO && terminalFd != STDERR_FILENO)
		return false;

#if defined(_WIN32) && defined(ENABLE_VIRTUAL_TERMINAL_PROCESSING)
	static const int fdMap[] = { STD_INPUT_HANDLE, STD_OUTPUT_HANDLE, STD_ERROR_HANDLE };
	...
	HANDLE hOut = GetStdHandle(fdMap[terminalFd]);
